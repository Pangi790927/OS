...
namespace keyboard
{
	static static_queue_t<uint32, max_queue_size + 1> keys_que;
	bool newKey = false;
	bool isInit = false;
}
...
void keyboard::init() {
	...
	KDBG("Will try to reset the keyboard driver");
	...
	if (self_test != SELF_TEST_PASSED) {
		KDBG("Self test failed with: %x\n", self_test);
		return ;
	}

	done = false;
	KDBG("Will try to set the scan code 2");
	for (int i = 0; i < 3 && !done; ++i) {
		ps2::send_data(SET_SCAN_CODE);
		uint8 rsp = ps2::read_data();
		ps2::send_data(2);
		rsp = ps2::read_data();
		...
		if (rsp == ACK) {
			...
			KDBG("Failed tp set scan code: %x", rsp);
			break;
		}
	}

	if (!done) {
		KDBG("Keyboard inactive\n");
		return ;
	}

	if (ps2::clear_data())
		KDBG("Keyboard data needed cleaning");

	KDBG("Keyboard init success");
}

void keyboard::irq () {
	uint8 key = inb(ps2::DATA_PORT);
	keys_que.push(key);
	newKey = true;
}
